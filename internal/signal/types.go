...
var signalTypeRegistry = map[string]SignalTypeInfo{
	...
	"native_extension_present": {
		...
		Group:             profile.SignalGroupPublication,
		ForgeryResistance: profile.ForgeryHigh,
		Description:       "Whether the latest published version ships native extensions. On gem: platform != 'ruby' (extconf.rb runs at install). On PyPI: a wheel carrying a concrete PEP 425 platform tag (manylinux / macosx / win / musllinux …) rather than the pure-Python 'any' — a compiled .so/.pyd that executes on dlopen at import. The gem/PyPI equivalent of cargo's build.rs or npm's postinstall.",
		Caveats: []string{
			"native extensions are common in legitimate packages (gem: nokogiri, ffi, pg; PyPI: numpy, scipy, cryptography, pydantic) — presence alone is not negative",
			"the signal flags a distinct attack surface: native code runs with full system access at install (gem) or import (PyPI wheel) time",
			"gem: platform is per-version metadata set by rubygems at publish time. PyPI: derived from the wheel filename's platform tag in the already-collected registry response — no wheel is opened, so the tag is what the publisher declared",
			"PyPI: the 2026-06 Miasma/Hades campaign shipped malicious code inside trojanized .abi3.so wheels — this signal sees that a native wheel is present, not what the compiled extension does",
		},
	},
	"native_extension_introduced": {
		...
		Group:             profile.SignalGroupPublication,
		ForgeryResistance: profile.ForgeryHigh,
		Description:       "Whether a native extension appeared in a recent version where prior versions were pure Ruby (gem) / pure-Python (PyPI). Longitudinal complement to native_extension_present — the gem/PyPI analog of build_script_introduced and the wheel-side parallel to sdist_only_introduced.",
		Caveats: []string{
			...
			"PyPI: catches the masquerade shape (a 'pure-Python' typosquat that suddenly compiles a .so) but NOT trojanization of an already-native package (e.g. the embiggen/ensmallen bioinformatics cluster), where a native wheel is expected every version — detecting that needs wheel-content inspection signatory does not perform",
		},
	},
